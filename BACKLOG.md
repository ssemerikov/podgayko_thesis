...
- [ ] T1.3 Теоретична рамка (EDM/LA, EQAVET, DSR) — наповнення підрозділу 1.1 (наразі stub).
- [ ] T1.4 Multi-LLM cluster naming (3 Claude + 3–4 Ollama + 1–2 OpenRouter) → метричний звіт `agreement_metrics.csv`; наповнення підрозділу 1.5.6 (наразі stub).
- [ ] T1.5 Підрозділ 1.6 «Імплікації» + кейс переміщених (Луганщина) + контекст реформ (війна / відновлення / ЄС) — наразі stub.
