...
- [ ] T1.1 PRISMA 2020 flow + checklist у методологію та додаток.
- [ ] T1.2 Огляд попередніх SLR (8–12 оглядів) → `bachelor_thesis/data/prior_reviews.csv` + порівняльна таблиця.
- [ ] T1.3 Теоретична рамка (EDM/LA, EQAVET, DSR) — підрозділ chapter1.
- [ ] T1.4 Multi-LLM cluster naming (3 Claude + 3–4 Ollama + 1–2 OpenRouter) → метричний звіт `agreement_metrics.csv`.
- [ ] T1.5 Розділ Implications + кейс переміщених (Луганщина) + контекст реформ (війна / відновлення / ЄС) — chapter4_implications.tex, chapter_displaced.tex, chapter_war_recovery.tex.
- [ ] T1.6 Перехід на DSTU 8302:2015 + розширення bib до 70–90 джерел з пріоритетом 2025–2026.
- [ ] T1.7 Анотація укр./англ. ≥200 слів, оновлений вступ, висновки.
