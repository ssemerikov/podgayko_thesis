...
\item бібліометрична методика з~прозорою стратегією пошуку
...
\node[draw,rounded corners,fill=orange!10,minimum width=8cm,minimum height=0.6cm,below=of svc] (rep) {Drizzle repositories: 8 модулів даних, 24~форми моніторингу};
...
\textbf{Реалізовано:}
\begin{itemize}\small
\item 13~HTML-сторінок (8~публ. + 5~закр.)
\item Tailwind CSS, Chart.js, Leaflet+OSM
\item Каталог 14~ЗПО + мапа релокації
\item KPI-дашборд, динаміка контингенту 2021--2026 з~маркером 24.02.2022
\item Кабінет ЗПО, 24~форми за~7~блоками
\item Дизайн-система: 6~кольорів, Inter+Lora
\item WCAG~2.1~AA, mobile-first
\end{itemize}
\end{column}
\begin{column}{0.55\textwidth}
\centering
...
% ----- 13. Оцінювання -----
\begin{frame}{Емпіричне оцінювання прототипу}
\begin{itemize}\small
...
\item Сутність моніторингу ЗПО~+~3~драйвери реформи №~4574-IX (війна, відбудова, ЄС)
\item Аналіз 5~нац.\ систем~$\to$~функціональна прогалина обласного рівня
\item 4~семантичні кластери глобальної літератури; $r>0{,}9$~VOSViewer~$\leftrightarrow$~Python
\end{itemize}
\textbf{Розділ~2 (концепція й~архітектура):}
\begin{itemize}\footnotesize
\item Концепція порталу «ПрофОсвіта~Луганщини»~+~14~переміщених закладів
\item ABAC JSON-AST: територіальна ієрархія + спеціалізація
\item SHA-256 хеш-ланцюг~+~ідемпотентний прийом 24~форм за~7~блоками
\end{itemize}
\textbf{Розділ~3 (веб-розробка):}
\begin{itemize}\footnotesize
\item Прототип: 13~HTML-сторінок, Tailwind/Chart.js/Leaflet, мапа релокації
\item Дизайн-система~+~WCAG~2.1~AA~+~mobile-first
\item Експертне оцінювання $n\geq 3$~+~SUS~$\to$~TRL~4--5; дорожня карта Q2~2026 -- Q1~2027
\end{itemize}
\end{frame}
