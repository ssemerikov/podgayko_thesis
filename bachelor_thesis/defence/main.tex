...
\node[draw,rounded corners,fill=blue!8,minimum width=8cm,minimum height=0.6cm] (ui) {Серверне відображення: EJS + htmx + Alpine + axe-core (доступність)};
\node[draw,rounded corners,fill=green!8,minimum width=8cm,minimum height=0.6cm,below=of ui] (api) {Fastify HTTP API + декоратор ABAC};
...
\node[draw,rounded corners,fill=orange!10,minimum width=8cm,minimum height=0.6cm,below=of svc] (rep) {Репозиторії Drizzle: 8 модулів даних, 24~форми моніторингу};
...
\textbf{Реалізовано:}
\begin{itemize}\small
\item 13~HTML-сторінок (8~публічних + 5~закритих)
...
\begin{itemize}\small
\item \textbf{Аудит-ланцюг:} 10\,000 оновлень + перевірка цілісності~$\to$~100\% цілісність
\item \textbf{ABAC:} матриця «роль~$\times$~дія~$\times$~ресурс» ($\sim$160~тестів) + наскрізні тести без~обхідних шляхів
\item \textbf{Двосторонній обмін з~Excel:} 8~шаблонів, повна відповідність для~валідних рядків
\item \textbf{Затримка:} цільовий $p_{95}<500$~мс (50~закладів~$\times$~1\,000~здобувачів)
\item \textbf{WCAG~2.1~AA:} axe-core на~9~маршрутах, 0~критичних порушень
\item \textbf{Експертне оцінювання $n\geq 3$:} SUS + 8 домен-специфічних питань + 4 відкриті відповіді + 1--2 поглиблені сесії
\item \textbf{Сценарний прохід:} 5~сценаріїв, включно з~кейсом переміщеного закладу
\end{itemize}
\end{frame}
...
\item \textbf{12~місяців} (\textit{повоєнне відновлення}): пілот у~3--5~областях; інтеграція з~ЄДЕБО (ETL); КЕП-підпис; розширення публічно-приватних партнерств за~моделями ЄС
\item \textbf{24~місяці} (\textit{гармонізація з~ЄС}): сумісність з~10~індикаторами EQAVET; машинне навчання для~передбачення відрахування з~пояснюваністю результатів; поширення на~всі~25~областей; передача МОН
\end{itemize}
\textbf{Адресні рекомендації:} МОН (стандарт переміщених, EQAVET); обласні департаменти (локальне розгортання, ризик-орієнтований підхід); керівники (юр./факт.\ адреса, масовий імпорт із~Excel); інспектори (спеціалізація ABAC, обмін досвідом).
\end{frame}
...
\item Сутність моніторингу ЗПО~+~3~чинники реформи №~4574-IX (війна, відбудова, ЄС)
...
\item Дизайн-система~+~WCAG~2.1~AA~+~стратегія «спочатку мобільний»
